...
def run_http_server(http_port, ws_port, ui_requests):
    kwargs = {
        "http_port": http_port,
        "ws_port": ws_port,
        "ui_requests": ui_requests,
    }
    http_server = HTTPServer(("", http_port), makeHTTPRequestHandler(handle_get_request, handle_post_request, kwargs))
...
class WebServer:
    ...
    def connect(self, http_port=8000, ws_port=8001):
        ...
        # Start HTTPServer
        self.http_server_process = Process(target=run_http_server, args=(http_port, ws_port, self._ui_requests))
        ...
        ws_server_thread = threading.Thread(target=self.ws_server.serve_forever, args=(self._initialize_client,))
        ws_server_thread.daemon = True
        ws_server_thread.start()
        print("Started WebSocket server on port %d\n" % (ws_port))

        # Start RedisMonitor
        print(f"Server ready. Listening for incoming connections at http://localhost:{http_port}.\n")
    ...
    def commit(self):
        ...
        for client in self.ws_server.clients:
            client.send(self.ws_server.encode_message({"update": self._key_vals, "delete": self._del_keys}))
